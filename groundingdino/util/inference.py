...
def predict(
    ...
    keep = nms(prediction_boxes, scores, 0.5)  # Use IoU threshold 0.5
    filtered_boxes = prediction_boxes[keep]
    filtered_logits = prediction_logits[keep]
    ...
    for logit in filtered_logits:
        ...
        valid_tokens = []
        for idx, (token_id, mask) in enumerate(zip(tokenized['input_ids'][0], text_mask)):
            if token_id in [tokenizer.cls_token_id, tokenizer.sep_token_id, tokenizer.pad_token_id]:
                continue
            if mask:
                valid_tokens.append(token_id.item())

        if valid_tokens:  # Check if the phrase is not empty
            phrase = tokenizer.decode(valid_tokens)
            if len(phrase) > 1:  # Ensure the phrase has more than one token
                phrases.append(phrase.replace('.', ''))

    return filtered_boxes, filtered_logits.max(dim=1)[0], phrases
