...
class GroundingDINOTrainer:
    def __init__(
        ...
        self.weights_dict_loss = {'loss_ce': class_loss_coef, 'loss_bbox': bbox_loss_coef * 2,
                                  'loss_giou': giou_loss_coef}
        self.criterion = SetCriterion(max_txt_len, self.matcher, eos_coef, losses)
        self.criterion.to(device)

    import torch
    def extract_foreground_features(self, pred_logits, pred_boxes):
        # 假设 pred_logits 的 shape 是 (batch_size, num_queries, num_classes)
        # 假设 pred_boxes 的 shape 是 (batch_size, num_queries, 4)

        # Step 1: 找到每个边界框得分最高的类别
        max_logits, _ = torch.max(pred_logits, dim=-1)  # shape: (batch_size, num_queries)

        # Step 2: 选择得分最高的类别对应的特征
        batch_size, num_queries, num_classes = pred_logits.shape
        max_features = pred_logits.view(-1, num_classes).max(dim=-1)[0].view(batch_size, num_queries)

        # Step 3: 计算前景特征的平均值
        foreground_features_avg = max_features.mean(dim=1)  # shape: (batch_size,)

        return foreground_features_avg
    ...
    def train_step(self, batch):
        ...
        # Extract and compute the average of bbox features
        # 确保 outputs 中包含 'pred_logits' 和 'pred_boxes'
        bbox_feature_avg = self.extract_foreground_features(outputs['pred_logits'], outputs['pred_boxes'])

        # Use the bbox feature average in the loss calculation
        # Assuming the model output has a key 'pred_logits' for classification logits
        # 确保 outputs['pred_logits'] 的维度正确，可以进行加法操作
        outputs['pred_logits'] += bbox_feature_avg.unsqueeze(1).unsqueeze(2)  # Add the average feature to logits

        loss_dict = self.criterion(outputs, targets, captions=captions, tokenizer=self.model.tokenizer)
        total_loss = sum(
            loss_dict[k] * self.weights_dict_loss[k] for k in loss_dict.keys() if k in self.weights_dict_loss)
...
def train(
    ...
    visualizer = GroundingDINOVisualizer(save_dir="visualizations_self")
...
if __name__ == "__main__":
    data_dict = {
        'train_dir': r"E:\Gong\new\Grounding-Dino-FineTuning-main0\stamen_dataset0\images\train_ss",
        'train_ann': r"E:\Gong\new\Grounding-Dino-FineTuning-main0\stamen_dataset0\train_ss.csv",
        'val_dir': r"E:\Gong\new\Grounding-Dino-FineTuning-main0\stamen_dataset0\images\val_ss",
        'val_ann': r"E:\Gong\new\Grounding-Dino-FineTuning-main0\stamen_dataset0\val_ss.csv"
    }
