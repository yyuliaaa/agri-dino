...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.5, max_rotation_angle=5,
                 ...
                 add_noise_rate=0.0001, flip_rate=0.5,
    ...
    def _addNoise(self, img):
        ...
        输出:
            加噪声后的图像array
        '''
        return random_noise(img, mode='gaussian', mean=0, var=0.0001, clip=True)
    ...
    def _cutout(self, img, bboxes, length=100, n_holes=1, threshold=0.5):
        ...
        for n in range(n_holes):
            ...
            while chongdie:
                ...
                chongdie = False
                for box in bboxes:
                    if self.cal_iou([x1, y1, x2, y2], box) > threshold:
                        chongdie = True
                        break
            if not chongdie:
                mask[y1: y2, x1: x2, :] = 0.
    ...
    def _rotate_img_bbox(self, img, bboxes, angle=5, scale=1.):
        ...
        for bbox in bboxes:
            ...
            if rot_img.max() == 0:
                print("Warning: All black image generated after rotation.")
    ...
    def _filp_pic_bboxes(self, img, bboxes):
        ...
        h, w, _ = img.shape
        print("Flip image shape:", flip_img.shape)
        sed = random.random()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_img_path', type=str, default=r'E:\Gong\ultralytics-main\data\images')
    parser.add_argument('--source_xml_path', type=str, default=r'E:\Gong\ultralytics-main\data\要增强')
    parser.add_argument('--save_img_path', type=str, default=r'E:\Gong\ultralytics-main\data\images_enhance')
    parser.add_argument('--save_xml_path', type=str, default=r'E:\Gong\ultralytics-main\data\xml_enhance')
    ...
    for parent, _, files in os.walk(source_img_path):
        files.sort()
        for file in files:
            if file <= 'DJI_0500.JPG':
                continue

            pic_path = os.path.join(parent, file)
            xml_file = file[:-4] + '.xml'
            xml_path = os.path.join(source_xml_path, xml_file)

            if not os.path.exists(xml_path):
                print(f"Skipping {file} because corresponding XML file {xml_file} does not exist.")
                continue
            ...
            while cnt < need_aug_num:
                ...
                if auged_img.max() == 0:
                    print(f"Warning: All black image generated for {file}")
                ...
                toolhelper.save_xml('{}_{}.xml'.format(_file_prefix, cnt + 1), save_xml_path, (save_img_path, img_name),
